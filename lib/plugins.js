...
function loadPlugin(name, config) {
  ...
  if (typeof plugin.COIN === 'undefined') {
    /* This plugin doesn't define which coin it works with.
     * In this case BTC is assumed to be the cryptocurrency. */
    configuredCoins['BTC'] = true;
  } else {
    pluginCoin = plugin.COIN;
    configuredCoins[pluginCoin] = true;
  }
...
function loadOrConfigPlugin(pluginHandle, pluginType, currency,
    onChangeCallback) {
  var result;
  if (typeof cachedConfig.exchanges.plugins.coins === 'undefined') {
    /* default to activeCoin ('BTC' */
    result = loadOrConfigPluginCoin(pluginHandle, pluginType, currency, onChangeCallback, activeCoin);
  } else {
  Object.keys(cachedConfig.exchanges.plugins.coins).forEach(function(coin) {
    result = loadOrConfigPluginCoin(pluginHandle, pluginType, currency,
                                    onChangeCallback, coin);
  });
  }
  return result;
}

function loadOrConfigPluginCoin(pluginHandle, pluginType, currency,
    onChangeCallback, coin) {
  var pluginChanged;
  var pname = (coin == 'BTC' ? '' : coin);
  var currentName = cachedConfig.exchanges.plugins.current[pname + pluginType];
  if (!currentlyUsedPlugins[coin]) {
    currentlyUsedPlugins[coin] = {};
  }
  pluginChanged = currentlyUsedPlugins[coin][pluginType] !== currentName;
  if (!currentName) pluginHandle[coin] = null;
    ...
    if (pluginHandle && !pluginChanged) pluginHandle[coin].config(pluginConfig);
    else {
      pluginHandle[coin] = loadPlugin(currentName, pluginConfig);
      currentlyUsedPlugins[coin][pluginType] = currentName
      logger.debug('plugin(%s, %s) loaded: %s', pluginType, coin,
                   pluginHandle[coin].NAME || currentName);
    }
  }

  if (typeof onChangeCallback === 'function')
    onChangeCallback(pluginHandle[coin], currency, coin);

  return pluginHandle;
}

exports.getActiveCoin = function() {
  return activeCoin;
};

exports.setActiveCoin = function(coin, cb) {
  var err = null;
  var code = 200;
  if (!configuredCoins[coin]) {
    code = 500;
    err = {msg: 'coin not configured'};
  } else {
    activeCoin = coin;
  }
  //var oldconfig = this.getConfig()
  //oldconfig.exchanges.plugins.current.coin = coin
  //this.configure(oldconfig)
  cb(err, {statusCode: code, coin: coin});
};
...
exports.configure = function configure(config) {
  ...
  loadOrConfigPlugin(
    ...
    function onTickerChange(newTicker, currency, coin) {
      tickerPlugin[coin] = newTicker;
      pollRate();
    }
  );
  ...
  loadOrConfigPlugin(
    ...
    'transfer',
    null,
    function onWalletChange(newWallet, currency, coin) {
      walletPlugin[coin] = newWallet;
      pollBalance();
    }
  );
  ...
  traderPlugin = loadOrConfigPlugin(
    ...
    'trade',
    null,
    function onTraderChange(newTrader, currency, coin) {
      traderPlugin[coin] = newTrader;
...
// TODO this really means sendCoins now, as it applies to all cryptocurrencies
function _sendBitcoins(toAddress, satoshis, cb) {
  var transactionFee = cachedConfig.exchanges.settings.transactionFee;
  walletPlugin[activeCoin].sendBitcoins(toAddress, satoshis, transactionFee, cb);
}
...
exports.cashOut = function cashOut(session, tx, cb) {
  ...
  walletPlugin[activeCoin].newAddress(tmpInfo, function(err, address) {
    if (err) return cb(err);
...
exports.fiatBalance = function fiatBalance() {
  ...
  var transferBalance = lastBalances.transferBalance[activeCoin];
...
function startTrader() {
  ...
  if (traderPlugin[activeCoin] && !tradeInterval) {
...
function pollBalance(cb) {
  if (!configuredCoins[activeCoin] || !walletPlugin[activeCoin]) {
    return cb && cb(new Error('coin not configured yet'));
  }
  logger.debug('collecting balance');
  
  var jobs = {
    transferBalance: walletPlugin[activeCoin].balance
  };
...
function pollRate(cb) {
  if (!configuredCoins[activeCoin] || !tickerPlugin[activeCoin]) {
    return cb && cb(new Error('coin not configured yet'));
  }
  logger.debug('polling for rates (%s)', tickerPlugin[activeCoin].NAME);
  tickerPlugin[activeCoin].ticker(deviceCurrency, function(err, resRates) {
    if (err) {
      logger.error(err);
      return cb && cb(err);
    }
...
function purchase(trade, cb) {
  traderPlugin[activeCoin].purchase(trade.satoshis, null, function(err) {
    if (err) return cb(err);
    pollBalance();
...
function executeTrades() {
  if (!traderPlugin[activeCoin]) return;
