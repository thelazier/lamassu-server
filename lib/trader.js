  ...
  transferExchange.newAddress(this.depositAccount, function (err, address) {
    ...
    self.db.summonTransaction(deviceFingerprint, tx, function (err, txRec) {
      ...
      if (!txRec) {
        ...
        // TODO: this needs to be done in polling
        transferExchange.monitorDepositAddress(address, confs, function (err, received) {
          var txId = tx.txId;
          if (err) {
            self.dispenseStatuses[deviceFingerprint] = {status: 'error', txId: txId};
            var errorStatus = err.name === 'timeout' ? 'depositTimeout' : 'depositError';
            self.db.reportTransactionError(tx, err.message, errorStatus);
            return logger.error(new Error(err));
          }

          if (received === 0) return;

          if (received < tx.satoshis) {
            var statusRec = {
              status: 'insufficientDeposit',
              txId: txId,
              deposit: received,
              expectedDeposit: tx.satoshis
            };
            self.dispenseStatuses[deviceFingerprint] = statusRec;
            // TODO self.db.insufficientDeposit(tx, received);
            return;
          }
          self.dispenseStatuses[deviceFingerprint] = {status: 'dispense', txId: txId};
          self.db.dispenseReady(tx);
        });
