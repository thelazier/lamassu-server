...
var Trader = module.exports = function (db) {
  ...
  this._sessionInfo = {};
  this.rateInfo = null;
  this.dispenseStatuses = {};
  this.poolAccount = 'pool';
  this.fundingAccount = 'funding';
...
Trader.prototype.configure = function (config) {
  ...
  transferExchangeConfig.poolAccount = this.poolAccount;

  if (argv['mock-bitcoind'])
    transferExchangeConfig.bitcoindConfigurationPath = argv['mock-bitcoind'];
...
// Monitors given address until any transaction comes in
Trader.prototype._monitorAddress = function _monitorAddress(address, cb) {
  var transferExchange = this.transferExchange;
  var confs = 0;
  var received = 0;
  var t0 = Date.now();
  ...
  function checkAddress(cb) {
    transferExchange.addressReceived(address, confs, function (err, _received) {
      if (err) logger.error(err);
  ...
  function handler() {
    ...
    if (received === 0) return cb(new Error('Timeout while monitoring address'));
    cb(null, received);
  }
...
// Waits for deposit and handles errors
Trader.prototype._waitDeposit = function _waitDeposit(deviceFingerprint, tx) {
  var self = this;
  this._monitorAddress(tx.toAddress, function (err, received) {
    var status = null;
    if (err) status = 'timeout';
    else if (received < tx.satoshis) status = 'insufficientDeposit';
    else status = 'fullDeposit';
    ...
    self.dispenseStatuses[deviceFingerprint] = depositStatus;
    // TODO self.db.dispenseReady(tx);
  });
};

// This is idempotent
// TODO: break this up
Trader.prototype.cashOut = function cashOut(deviceFingerprint, tx, cb) {
  var self = this;
  var transferExchange = this.transferExchange;
  transferExchange.newAddress(this.depositAccount, function (err, address) {
    ...
    self.db.summonTransaction(deviceFingerprint, tx, function (err, txRec) {
      if (err) return cb(new Error(err));
      if (!txRec) {
        self._waitDeposit(deviceFingerprint, tx);
        return cb(null, address);
      }

      // Transaction already exists
      cb(null, txRec.to_address);
    });
  });
};

Trader.prototype.depositAck = function depositAck(deviceFingerprint, tx, cb) {
/* TODO
  var self = this;
  var status = this.dispenseStatuses[deviceFingerprint];

  if (status === 'dispense') {
    this.db.dispensing(tx, function (err) {
      if (err) return cb(new Error(err));
      self.dispenseStatuses[deviceFingerprint] = null;
      return cb();
    });    
  }
*/

  this.dispenseStatuses[deviceFingerprint] = null;
  cb();
};

Trader.prototype.dispenseStatus = function dispenseStatus(deviceFingerprint) {
  return this.dispenseStatuses[deviceFingerprint];
};
